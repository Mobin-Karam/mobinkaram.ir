    ...
    <div className="space-y-8">
      ...
      <LazySection minHeight={200} skeleton={<Skeleton className="h-48" />}>
        <div className="card p-5 space-y-3">
          <h3 className="text-lg font-semibold text-[color:var(--foreground)]">Now</h3>
          <div className="grid gap-3 md:grid-cols-3">
            {now.map((block) => (
              <div key={block.title} className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-3">
      ...
      <LazySection minHeight={220} skeleton={<Skeleton className="h-52" />}>
        <div className="card p-5 space-y-3">
          <h3 className="text-lg font-semibold text-[color:var(--foreground)]">Stack highlights</h3>
          <div className="grid gap-3 md:grid-cols-2">
            {stackReasons.slice(0, 6).map((item) => (
              <div key={item.name} className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] p-3">
                ...
                <p className="mt-2 text-sm text-[color:var(--muted)]">{item.why}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <LazySection minHeight={200} skeleton={<Skeleton className="h-48" />}>
        <div className="card p-5 space-y-3">
          <h3 className="text-lg font-semibold text-[color:var(--foreground)]">About snapshot</h3>
            ...
            {quickLinks.map((link) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-[color:var(--muted)]">{link.desc}</p>
                </div>
